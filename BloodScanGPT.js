...
const openai = new OpenAI({ apiKey: environment.openAi.apiKey });
    ...
    const prompt = `Extract blood parameters, their values, range of values, 
    ...
    exports.bloodMarkersExtraction = functions
        .region("europe-west1")
        .runWith({ timeoutSeconds: 180 })
        .https.onCall(async (data) => {
            try {
                const response = await axios.get(data.bloodReportURL, { responseType: 'arraybuffer' });
                const text = await extractTextFromPDF(response)
                return (text !== "") ? await extractBloodParamsWithGPT3(text) : await performOCRWithGPT4(await convertPdfToImages(response))
            } catch (error) {
                console.error('Error converting PDF to images:', error);
            }
        })

    async function performOCRWithGPT4(base64Images) {
        try {
            const messages =
                [{
                    role: "user",
            ...
            const data = JSON.parse(response.choices[0].message.content.trim().slice(7, -3).trim())
    ...
    async function extractBloodParamsWithGPT3(text) {
        try {
            const conversation = [
    ...
    async function convertPdfToImages(response) {
        try {
